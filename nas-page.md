...
I chose **Pi OS Lite** for the NAS because the package I used called **OpenMediaVault** doesn't integrate well with desktop environments. As a result, I ended up not choosing the full version of Pi OS, which includes a desktop interface. Instead, I used Zsh to navigate and program the system.
...
NAS was soon up and running. Next, I gained access to the board using SSH and installed OpenMediaVault to monitor storage usage, network traffic and system logs. My drive used a HFS+ file system which caused compatibility issues, so I reformatted it to EXT4. I also ended up skipping RAID configuration for simplicity. 

Once you install OMV, it overrides a lot of the default functionalities of the Pi, and it took me some time to realize this and re-enable access from inside OMV. 

With the drive finally mounted to OMV and email alerts enabled using SMTP, the NAS was complete. Later on, I added VPN so I could securely access files from outside the house.
